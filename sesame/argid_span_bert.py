...
my_dict_train_examples = get_all_span_reduced_bert_embeddings(trainexamples)

if options.mode in ["train", "refresh"]:
    devexamples, _, _ = read_conll(DEV_CONLL, options.syn)
    out_conll_file = "{}predicted-{}-argid-dev.conll".format(model_dir, VERSION)
    my_dict_dev_examples = get_all_span_reduced_bert_embeddings(devexamples)
    ...
    my_dict_instances_examples = get_all_span_reduced_bert_embeddings(instances)
else:
...
def identify_fes(factor_exprs, goldfes=None, testidx=None):
    dy.renew_cg()
...
if options.mode in ["train", "refresh"]:
    ...
    for epoch, _ in enumerate(epoch_iterator):
        ...
        for idx, trex in enumerate(train_iterator, 1):
            ...
            if USE_PTB_CONSTITS and type(trex) == Sentence:  # a PTB example
                ...
                trexloss, taggedinex = identify_fes(my_dict_train_examples[trex],
            ...
            if (idx - 1) % DEV_EVAL_EPOCHS == 0 and idx > 1:
                ...
                for devex in devexamples:
                    ...
                    dargmax = identify_fes(my_dict_dev_examples[devex])
    ...
    for tidx, testex in enumerate(devexamples, 1):
        ...
        if tidx % 100 == 0:
            sys.stderr.write(str(tidx) + "...")
        testargmax = identify_fes(my_dict_dev_examples[testex],
                                  testidx=tidx - 1)
        testpredictions.append(testargmax)
    ...
    for instance in instances:
        ...
        prediction = identify_fes(my_dict_instances_examples[instance])
